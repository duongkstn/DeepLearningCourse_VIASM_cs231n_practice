...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        loss, dout = temporal_softmax_loss(out_temporal, captions_out, mask, verbose=False)
    ...
    def sample(self, features, max_length=30):
        ...
        x = x.reshape(1,x.shape[0]) #1 is T
        ...
        for i in range(max_length):
            ...
            next_h = next_h.reshape(next_h.shape[0], 1, next_h.shape[1])  #because of trick, we need 3D array
            out_temporal, _ = temporal_affine_forward(next_h, W_vocab, b_vocab) 
            out_temporal = out_temporal.reshape(out_temporal.shape[0],out_temporal.shape[2])
            x = np.argmax(out_temporal, axis = 1)
            captions[:, i] = x
            prev_h = next_h.reshape(next_h.shape[0], next_h.shape[2]) #because of trick, we need remove dim = 1
            prev_c = next_c
            x = W_embed[x]
        ############################################################################
        #                             END OF YOUR CODE                             #
