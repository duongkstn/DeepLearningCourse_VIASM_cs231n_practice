...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        Hcomma = int(1 + (Hnew - HH) / stride)
        Wcomma = int(1 + (Wnew - WW) / stride)

        Hpool = int(1 + (H - 2) / 2)
        Wpool = int(1 + (W - 2) / 2) #2 is stride of maxpool
                

        self.params['W2'] = np.random.normal(0,weight_scale,(F * Hpool * Wpool, hidden_dim)) #because C after Convole is F
